...
import { partners as initialPartners } from "@/data/mock-data";
import type { Partner } from "@/types";

function recalculateEquity(partners: Partner[]): Partner[] {
  const totalBalance = partners.reduce((sum, p) => sum + p.totalBalance, 0);
  if (totalBalance === 0) return partners;

  return partners.map((p) => ({
    ...p,
    ownershipPercentage:
      Math.round((p.totalBalance / totalBalance) * 10000) / 100,
  }));
}

export default function PartnersPage() {
  const [partners, setPartners] = useState<Partner[]>(() =>
    recalculateEquity(initialPartners)
  );
  ...
  const deletePartner = useCallback((id: string) => {
    setPartners((prev) => {
      ...
      return recalculateEquity(remaining);
    });
  ...
  return (
    <AppShell>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteTarget && deletePartner(deleteTarget.id)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-primary p-6 rounded-sm flex flex-col justify-between h-32 relative overflow-hidden">
          ...
          <div>
            ...
            <span className="text-xs text-on-primary/70 mr-3">
              4 طلبات معلقة
            </span>
          </div>
        </div>
