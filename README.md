...
3. **Run `supabase/migrations/014_auth_rls_reconcile.sql`** — the one file to run on an already-deployed DB. It links every auth account to its partner row by email, adds `is_gp()`, and replaces the admin-only policies: authenticated partners read everything (`transactions`: own rows only), **only the GP writes**, anon reads nothing. (Supersedes `013` for a live DB; `013` assumed policy names that don't exist there.)
4. **Redeploy** — enforcement is **on by default** in code now, so no env var is needed: the web app starts requiring sign-in on the next deploy. The GP gets the full terminal; each LP lands on their own details page (their stake, positions, and account statement). For the APK, rerun the Android APK workflow and distribute the new build. *(To temporarily disable sign-in, set `NEXT_PUBLIC_AUTH_ENFORCED=0` for the web / a Repository Variable `AUTH_ENFORCED=0` for the APK.)*
